...
*Fig. 1. Grid-Following overview.*

**Grid-Following Inverter (GFL)** เป็นกลยุทธ์การควบคุมที่ใช้กับอินเวอร์เตอร์ในระบบผลิตไฟฟ้าจากพลังงานหมุนเวียน เช่น **โซลาร์เซลล์ (PV), กังหันลม (Wind Turbine) หรือ ระบบกักเก็บพลังงานด้วยแบตเตอรี่ (Battery Energy Storage)** อินเวอร์เตอร์เหล่านี้จะทำงานโดยการ ซิงโครไนซ์ (synchronize) ตัวเองกับกริดที่มีอยู่ [1]
...
*Fig. 2. Grid-Following Control diagram*

หลักการทำงานของ Grid-Following Inverter มักอธิบายผ่านโครงสร้างการควบคุม (control structure) ที่แบ่งออกเป็น 3 ชั้นหลัก ๆ ดังนี้ [1]:  
...
2. **วงจรควบคุมส่วนนอก (Outer Control Loop)**

 Use Active Power | Use $$\large V_{dc}$$ 
| Use Active Power | Use V<sub>dc</sub> |
| :---------------: | :----------------: |
| <p align="center"><img src="Fig%20material/model-GFL_Power_loop1.png" width="600"></p> | <p align="center"><img src="Fig%20material/model-GFL_outer_loop.png" width="600"></p> |
| <div align="center"><em>(a)</em></div> | <div align="center"><em>(b)</em></div> |

*Fig. 3. Outer Control Loop where (a) Active Power. (b) DC Voltage.*


   กำลังไฟฟ้าจริงและกำลังรีแอกทีฟสามารถคำนวณบนแกน dq ผ่านสมการ
   <p>
      $$
      \large P^* = \frac{3}{2}(V_d I_d+V_q I_q) \quad Q^* = \frac{3}{2}(V_q I_d- V_d I_q)
      $$
   </p>
   โดยที่:
   ...
   จากกำลังที่วัดได้จะเปลี่ยนเป็นกระแสอ้างอิงบนแกน d ได้ผ่านสองสมการคือ
   <p>
      $$
      \large I_{d_{ref}} = (P_{ref}-P^*)*(K_p+\frac{K_i}{s}) \quad or \quad I_{d_{ref}} = (V_{DC_{ref}}-V_{DC}^*)*(K_p+\frac{K_i}{s})
      $$
      $$
      \large I_{q_{ref}} = (Q_{ref}-Q^*)*(K_p+\frac{K_i}{s})
      $$
   </p>
   โดยที่:
   ...
   - $V_{DC_{ref}},V_{DC}^*$ = แรงดันกระแสตรงที่ต้องการและแรงดันกระแสตรงที่วัดได้
...
*Fig. 4 Inner Control Loop*
   ...
   หลังจากนั้นจึงถูกแปลงกับให้กลับไปใน abc coordinate แล้วจึงไปยังวงจร PWM เพื่อสร้างสัญญาณควบคุมสวิตช์ของอินเวอร์เตอร์  
...
<p>
   ...
   ![Modified IEEE 9-bus test system](Fig%20material/R1sys_fig.PNG)  
   *Fig. 5. System Figure in [2].*
   ...
   **1. การตอบสนองเมื่อปิดฟังก์ชัน Frequency Droop Support**
   ...
   <img src="Fig%20material/R1Fig1.PNG" alt="Diagram" width="600">   
   
   *Fig. 6. Power deviation, frequency, and ROCOF for the test with GFL frequency droop grid support disabled. [2]*
   ...
   **2. การตอบสนองเมื่อเปิดฟังก์ชัน Frequency Droop Support**  
   ...
   <img src="Fig%20material/R1Fig2.PNG" alt="Diagram" width="600">  

   *Fig. 7. Power deviation, frequency, and ROCOF for the test with GFL frequency droop grid support enabled. [2]*
   ...
   *Fig. 8. Fault behavior of grid-following inverter active power and reactive power [2]*
...
---
   ...
   อย่างไรก็ตาม แม้ GFL จะมีข้อดีที่ **ราคาถูก** **ออกแบบง่าย** และ**ตอบสนองรวดเร็ว** แต่จุดอ่อนที่ซ่อนอยู่ในโครงสร้างการควบคุมแบบ “**ตามกริด**” กลับถูกเปิดเผยอย่างชัดเจนในเหตุการณ์ไฟดับครั้งใหญ่ของคาบสมุทรไอบีเรีย (Iberian Peninsula Blackout) เมื่อเดือน เมษายน ปี 2025 ซึ่งทำให้ประชาชนหลายล้านคนในสเปนและโปรตุเกสไม่มีไฟฟ้าใช้เป็นเวลาหลายชั่วโมง [4], [5], [6]
   <p>
      
   เหตุการณ์เริ่มขึ้นเมื่อวันที่ **28 เมษายน 2025** ขณะที่สายส่งแรงดันสูง 400 kV ซึ่งเชื่อมระหว่างสเปนกับฝรั่งเศสเกิด **การขัดข้องและตัดวงจรอัตโนมัติ (tripping)** พลังงานไฟฟ้าที่เคยไหลเข้าจากฝรั่งเศสหายไปในทันที ส่งผลให้แรงดันและความถี่ของระบบไฟฟ้าเริ่มสั่นสะเทือน

   ในช่วงเวลานั้น สเปนมีสัดส่วนพลังงานหมุนเวียนมากกว่า 60%  ซึ่งส่วนใหญ่เชื่อมต่อผ่าน **GFLs** อินเวอร์เตอร์เหล่านี้ต้อง “ตามกริด” โดยอาศัยแรงดันและความถี่อ้างอิงจากระบบหลัก เมื่อแรงดันกริดแปรปรวน PLL ของ GFL จึงเริ่มสูญเสียการซิงโครไนซ์ และอินเวอร์เตอร์จำนวนมากถูกสั่งให้ **ตัดตัวเองออกจากระบบ (protective trip)** เพื่อป้องกันความเสียหาย

   การหยุดทำงานของ GFL เกิดขึ้นเกือบพร้อมกันทั่วประเทศ ส่งผลให้กำลังผลิตหายไปกว่า **15 GW**, ความถี่ลดต่ำกว่า **49 Hz**, และเกิดการ **ตัดวงจรลูกโซ่ (cascading trip)** ระบบไฟฟ้าจึงเข้าสู่ภาวะ **blackout ภายในเวลาไม่ถึง 30 วินาที** ก่อนที่หน่วยงาน **Red Eléctrica de España (REE)** จะค่อย ๆ ฟื้นฟูระบบโดยใช้ synchronous generators ที่เหลืออยู่

   </p>

   **บทเรียนสำคัญจากเหตุการณ์นี้คือ**  
   
   แม้ GFL จะเหมาะสมกับระบบกริดปกติ เพราะราคาถูกและควบคุมง่าย แต่เมื่อระบบสูญเสียแรงดันอ้างอิงหรือแรงเฉื่อย (inertia) ต่ำ  GFL ไม่สามารถรักษาการเชื่อมต่อได้และจะล้มตามกริดทันที
   
   | จุดเด่นของ GFL | ข้อจำกัดที่เกิดในเหตุการณ์ไฟดับ |
   |------------------|-------------------------------------|
   | โครงสร้างควบคุมง่าย ใช้ PI หรือ PR controller | ขาดแรงเฉื่อยไฟฟ้า (ไม่มี inertia) |
   | ต้นทุนต่ำ ตอบสนองเร็ว | ขึ้นอยู่กับแรงดันอ้างอิงของกริด — ถ้ากริดล้ม GFL ล้มตาม |
   | เหมาะกับระบบกริดแข็งแรง | ไม่เสถียรใน weak grid หรือช่วง fault |

   หลังเหตุการณ์ไฟดับ สเปนและสหภาพยุโรปได้เสนอให้เพิ่มสัดส่วนของ  
   **GFM** ซึ่งสามารถ “กำหนดแรงดันและความถี่ของกริดได้เอง”  
   โดยไม่ต้องพึ่งพาแรงดันจากภายนอก  
   GFM ทำหน้าที่เสมือน **Virtual Synchronous Machine (VSM)**  
   ให้แรงเฉื่อยไฟฟ้าและรักษาเสถียรภาพเมื่อเกิดความรบกวนในระบบ
   
   แนวทางใหม่จึงมุ่งสู่ระบบ **Hybrid GFM + GFL**  
   โดยให้ GFM เป็นผู้นำ (กำหนดแรงดันและความถี่)  
   และให้ GFL เป็นผู้ตาม (จ่ายพลังงานตามสัญญาณจาก GFM)  
   เพื่อให้ระบบไฟฟ้ามีความมั่นคงและยืดหยุ่นมากขึ้น
   
   เหตุการณ์ไฟดับในสเปนแสดงให้เห็นว่า  
   - อินเวอร์เตอร์ที่ “เร็วและราคาถูก” อาจไม่เพียงพอ หากขาดความสามารถในการ “สร้างกริดได้ด้วยตัวเอง”
   
   แม้ **GFL** จะยังคงมีบทบาทสำคัญในระบบพลังงานหมุนเวียน  
   แต่เมื่อแรงเฉื่อยของระบบลดลงอย่างต่อเนื่อง  
   การพัฒนาและบูรณาการ **Grid-Forming control** จึงเป็นสิ่งจำเป็น  
   เพื่อให้พลังงานหมุนเวียนสามารถ  
   ไม่เพียงแค่ “ตามกริด” — แต่ยัง “สร้างกริด” ได้ในยามที่โลกต้องการเสถียรภาพที่สุด [7]
